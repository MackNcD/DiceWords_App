...
from customtkinter import CTkButton, CTkSlider, CTkSegmentedButton, CTkSwitch, CTkLabel
...
entry = tk.Text(app, 
                   ...
                   #font=None
                   foreground="#0C0E12",
                   font=('Helvetica, 12'), background="#0C0E12",
                   width=48, height=8,                           #CTkTextbox this and above
                   wrap="word",
                   #width=55, height=9,
                   #width=45, height =7,
                   )
...
output_text = tk.Text(app, font=('Segoe UI Variable Small Semibold', 20),
                   #height=5, width=32,
                   foreground=bg,background=text,
                   width=29, height=3,
                   wrap="word")
output_text.place(x=xx+x4+x7, y=yy+y4+y7-10)
...
segemented_button = customtkinter.CTkSegmentedButton(app, values=["Variegated", "Locked In"],command=on_off_radio, variable=var_radio,
...
def change_color_command(event=None):
    ...
    # Clear previous tags
        ...
        while True:
            ...
            if not end:
                break
            entry.tag_config('placeholder', background=bg, foreground="#B4D862")
